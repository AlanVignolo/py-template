...
def parsear_valor(raw: str) -> float:
    try:
        ...
        raise DatosInvalidosError("El valor proporcionado no es un numero valido: %s" %raw) from e

def validar_rango(valor: float) -> float:
    if valor < 0:
        raise DatosInvalidosError("El valor debe ser mayor o igual a cero: %s" %valor)
    return valor
